...
@app.route('/<path:path>')
def index(path):
    policy_loader = config.get("policy_loader_fn")
    policy = policy_loader()
    return render_template('index.html.j2',
        policy=yaml.dump(policy),
    )

def start_server(host, port, policy_loader_fn, accept_proxy_headers=False, logger=None):
    if accept_proxy_headers:
        app.wsgi_app = ProxyFix(app.wsgi_app, x_for=1, x_proto=1, x_host=1, x_port=1, x_prefix=1)
    config["policy_loader_fn"] = policy_loader_fn

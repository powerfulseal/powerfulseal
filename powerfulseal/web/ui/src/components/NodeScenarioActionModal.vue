<!--
NodeScenarioActionModal is a component used for *editing* actions. The model
...
export default {
  DEFAULT_ACTION,
  name: 'node-scenario-action-modal',
  data () {
    return {
  ...
  computed: {
    options () {
      return Object.keys(constants.NODE_ACTION_TYPES).map(key => {
        let index = constants.NODE_ACTION_TYPES[key]
        let text = constants.NODE_ACTION_TYPE_LABELS[index]

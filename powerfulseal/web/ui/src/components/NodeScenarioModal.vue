<!--
NodeScenarioModal is a component used for *adding new and editing* node
...
`openModal(scenario)`. When creating a modal, you may pass in DEFAULT_NODE_SCENARIO.
    ...
    <div>
        <b-modal size="lg"
            ...
            <b-form-group horizontal
                          ...
                          breakpoint="md"
                          label="Scenario Name"
                          label-for="nodeScenarioName">
                <b-form-input id="nodeScenarioName"
                              type="text"
            ...
            <b-form-group>
                ...
                <b-table :items="modifiedScenario.matchers"
                         :fields="tableFields.matcherAndFilter">
                    ...
                    <template slot="actions" slot-scope="row">
                        <b-button size="sm"
                                  class="mr-1"
                                  variant="primary"
                                  @click.stop="editMatcher(row.index)">
                            Edit
                        </b-button>
                        <b-button size="sm"
                                  variant="danger"
                ...
                <b-form inline>
                    <label class="sr-only" for="matcherPropertyName">Name</label>
                    <b-input class="mb-2 mr-sm-2 mb-sm-0"
                             id="matcherPropertyName"
                             v-model="formFields.newMatcherName"
                             placeholder="Property Name"></b-input>

                    <label class="sr-only" for="matcherPropertyValue">Value</label>
                    <b-input class="mb-2 mr-sm-2 mb-sm-0"
                             id="matcherPropertyValue"
                             v-model="formFields.newMatcherValue"
                             placeholder="Property Value"></b-input>
                    <b-button variant="primary"
                              @click.stop="addMatcher()">
                        Add
                    </b-button>
                </b-form>
            </b-form-group>
            ...
            <b-table :items="modifiedScenario.actions"
                ...
                <template slot="type" slot-scope="row">{{constants.NODE_ACTION_TYPE_LABELS[row.value]}}</template>
        ...
        <node-scenario-matcher-modal ref="nodeScenarioMatcherModal"
                                     @hidden="handleSubModalHidden"
                                     @updated="handleMatcherUpdated"/>
        <node-scenario-filter-modal ref="nodeScenarioFilterModal"
                                    @hidden="handleSubModalHidden"
                                    @updated="handleFilterUpdated"/>
        <node-scenario-action-modal ref="nodeScenarioActionModal"
                                    @hidden="handleSubModalHidden"
...
import NodeScenarioMatcherModal from './NodeScenarioMatcherModal'
import NodeScenarioFilterModal from './NodeScenarioFilterModal'
import NodeScenarioActionModal from './NodeScenarioActionModal'
import Vue from 'vue'

const DEFAULT_NODE_SCENARIO = {
  name: '',
  matchers: [],
  filters: [],
...
export default {
  components: {
    NodeScenarioMatcherModal,
    NodeScenarioFilterModal,
    NodeScenarioActionModal
  },
  DEFAULT_NODE_SCENARIO,
  name: 'node-scenario-modal',
  data () {
    return {
      constants,
      modifiedScenario: _.cloneDeep(DEFAULT_NODE_SCENARIO),
      tableFields: {
        matcherAndFilter: [
  ...
  methods: {
    ...
    addMatcher () {
      this.modifiedScenario.matchers.push({
        name: this.formFields.newMatcherName,
        value: this.formFields.newMatcherValue
      })

      this.formFields.newMatcherName = ''
      this.formFields.newMatcherValue = ''
    },
    editMatcher (index) {
      this.$refs.modal.hide()
      this.modalOptions.selectedMatcherIndex = index
      this.$refs.nodeScenarioMatcherModal.openModal(this.modifiedScenario.matchers[index])
    },
    deleteMatcher (index) {
    ...
    editFilter (index) {
      ...
      this.$refs.nodeScenarioFilterModal.openModal(this.modifiedScenario.filters[index])
    },
    deleteFilter (index) {
    ...
    editAction (isNewAction, index) {
      ...
      if (isNewAction) {
        this.$refs.nodeScenarioActionModal.openModal(NodeScenarioActionModal.DEFAULT_ACTION)
      } else {
        this.modalOptions.selectedActionIndex = index
        this.$refs.nodeScenarioActionModal.openModal(this.modifiedScenario.actions[index])
      }
    },
    deleteAction (index) {
